...
public function generatePDFditolak($id)
{
    // Mengambil parameter kode perusahaan
    $param['kodeperus'] = $id;
    ...
    $query = DB::table('detail_biaya_operational_proyek')
        ->where('fk_header_biaya_operational', $id)
        ->where('cek_status_detail_biaya_operational_proyek', 1)
        ->where('cek_approval_detail_biaya_operational_proyek', 0);
    
    // Ambil data dengan paginasi
    $data = $query->orderBy('created_at', 'desc')->paginate(50000);

    // Mengambil budget dari header biaya operational proyek
    $budget = DB::select("SELECT budget_biaya_operational_proyek as b FROM header_biaya_operational_proyek WHERE kode_biaya_operational_proyek = '$id'");
    
    // Menghitung total harga detail biaya operational proyek
    $sum = DB::select("SELECT SUM(db.harga_detail_biaya_operational_proyek) as a FROM detail_biaya_operational_proyek db WHERE db.fk_header_biaya_operational = '$id' AND cek_approval_detail_biaya_operational_proyek = 0 AND cek_status_detail_biaya_operational_proyek = 1");
    ...
    // Load view untuk PDF
    $pdf = PDF::loadView('printreport.reportdetailbiayaoperationalproyekyangditolak', $param, compact('data'))
        ->setPaper('a4', 'landscape'); // Mengatur ukuran kertas ke A4 dan orientasi landscape
